...
$("#dountilv2").on("click", function () {
    $.post(window.baseUrl + "/dountil/v2", {until: 5}).done(function (data) {
        handle("#dountilv2_response", "not ok - without what", "Dountil", data);
    }).error(function (err) {
        handle("#dountilv2_response", "OK - without what");
    });
    $.post(window.baseUrl + "/dountil/v2/sum", {}).done(function (data) {
        if("error" in data && data["error"] == "Please provide a number!") {
            handle("#dountilv2_response", "OK - without number");
        } else {
            handle("#dountilv2_response", "not ok - without number", "Dountil", data);
        }
    }).error(function (err) {
        handle("#dountilv2_response", "not ok - without number", "Dountil", err);
    });
    $.post({url: window.baseUrl + "/dountil/v2/sum", data: JSON.stringify({until: 7}), contentType: 'application/json; charset=utf-8'}).done(function (data) {
        if("result" in data && data["result"] == 28) {
            handle("#dountilv2_response", "OK - with /sum until=7");
        } else {
            handle("#dountilv2_response", "not ok - with /sum until=7", "Dountil", data);
        }
    }).error(function (err) {
        handle("#dountilv2_response", "not ok - with /sum until=7", "Dountil", err);
    });
    $.post({url: window.baseUrl + "/dountil/v2/factor", data: JSON.stringify({until: 4}), contentType: 'application/json; charset=utf-8'}).done(function (data) {
        if("result" in data && data["result"] == 24) {
            handle("#dountilv2_response", "OK - with /factor until=4");
        } else {
            handle("#dountilv2_response", "not ok - with /factor until=4", "Dountil", data);
        }
    }).error(function (err) {
        handle("#dountilv2_response", "not ok - with /factor until=4", "Dountil", err);
    });
});